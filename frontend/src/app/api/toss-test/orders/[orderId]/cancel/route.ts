...
export async function POST(
  ...
  try {
    const result = await fetchWithSession(request, (userId, accessToken) => fetch(
      backendUrl(
        `/api/v1/users/${userId}/integrations/toss-payments/test-orders/${encodeURIComponent(orderId)}/cancel`,
      ),
      {
        method: "POST",
        headers: {
    ...
    return NextResponse.json({ message: "Toss 테스트 결제를 취소하지 못했습니다." }, { status: 503 });
  }
}
