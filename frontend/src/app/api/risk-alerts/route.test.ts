...
function request(query: string, cookies = session) {
  return new NextRequest(`http://localhost/api/risk-alerts?${query}`, {
    headers: { cookie: cookies },
  });
}

describe("risk alert BFF route", () => {
  ...
  it("rejects invalid filters before calling the backend", async () => {
    ...
    expect((await GET(request("month=2026-08&minimumLevel=LOW"))).status).toBe(400);
  ...
  it("forwards the level and cursor for the authenticated user", async () => {
    const backend = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify(page), { status: 200 }),
    );

    const response = await GET(request("month=2026-08&minimumLevel=HIGH&cursor=next-page"));

    expect(response.status).toBe(200);
    expect(await response.json()).toEqual(page);
    expect(backend).toHaveBeenCalledWith(
      `http://localhost:8080/api/v1/users/${userId}/risk-alerts?month=2026-08&minimumLevel=HIGH&size=20&cursor=next-page`,
  ...
  it("returns a retryable response when the backend cannot be reached", async () => {
    ...
    expect(await response.json()).toEqual({ message: "위험 알림을 불러오지 못했습니다." });
  });
});
