...
class EmailVerificationPersistenceAdapter implements EmailVerificationStore {
    ...
    public Optional<AccountTarget> findAccount(UUID userId) {
        return accounts.findById(userId).map(a -> new AccountTarget(a.getId(), a.getEmail(), a.getEmailVerifiedAt() != null));
    }
    ...
    public Optional<Challenge> findChallenge(UUID userId) {
        return challenges.findById(userId).map(c -> new Challenge(c.getCodeHash(), c.getExpiresAt()));
    }
